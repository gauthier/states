...
trait FactoryTrait
{
     ...
     * @var string[]
     */
    protected $fullStatesClassesNamesList;
     ...
     * @return string[]
     */
    protected function getFullStateClassNameList()
    {
        if (empty($this->fullStatesClassesNamesList)) {
            $fullStateClassNameList = [];

            foreach ($this->listStatesByClasses() as $stateName => $finderLoader) {
                $className = $finderLoader->getStatedClassName().'\\'.Loader\FinderInterface::STATES_PATH.'\\'.$stateName;
     ...
     * @return string
     */
    protected function computeStateAlias($loadingStateName, Loader\FinderInterface $finderLoader)
    {
    ...
    public function startup($proxyObject, $stateName = null)
    {
        ...
        foreach ($statesList as $loadingStateName => $finderLoader) {
            ...
            $stateObject->setDIContainer($diContainerObject)
                ...
                ->setStateAliases($this->computeStateAlias($loadingStateName, $finderLoader));
